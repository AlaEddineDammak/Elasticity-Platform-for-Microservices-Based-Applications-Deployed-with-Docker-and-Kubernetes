...
<body id="page-top">
    ...
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Pods List</h6>
    </div>
    <div class="card-body">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <table class="table table-bordered dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
                            ...
                            <tbody>
                                <?php
                                // Commande kubectl pour obtenir les pods au format JSON
                                $command = "sudo -u aladin_257 kubectl get pods --namespace=default -o json";
                                ...
                                foreach ($data['items'] as $pod) {
                                    ...
                                    $ready = isset($pod['status']['containerStatuses'][0]['ready']) ? ($pod['status']['containerStatuses'][0]['ready'] ? 'Ready' : 'Not Ready') : 'N/A';
                                    $restarts = isset($pod['status']['containerStatuses'][0]['restartCount']) ? $pod['status']['containerStatuses'][0]['restartCount'] : 'N/A';
                                    $creationTimestamp = strtotime($pod['metadata']['creationTimestamp']);
                                    $age = humanTiming($creationTimestamp); // Fonction pour convertir le timestamp en format humain

                                    // Afficher les données du pod dans une ligne du tableau HTML
                                    echo '<tr>
                                            <td>' . $name . '</td>
                                            <td>' . $status . '</td>
                                            <td>' . $ready . '</td>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response === 'success') {
                                ...
                                alert('Failed to delete pod ' + podName + '. Please try again later.');
                            }
                        }
                    });
                }
